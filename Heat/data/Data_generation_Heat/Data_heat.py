...
epsilon = 1
sigma = 1
N = 64  # number of intervals, giving N+1 = 65 grid points
L = 20
dx = L / N  # h = a/J = 20/64
x = np.linspace(0, L, N + 1)  # 65 points from 0 to 20, 64 intervals
endTime = 1.0
timesteps = 500
...
function heat_drift_jl(du, u, p, t)
    epsilon, dx, sigma = p
    dx2 = dx^2
    du[1] = epsilon * (-2u[1] + 2u[2]) / dx2
    for i in 2:{N}
        du[i] = epsilon * (u[i-1] - 2u[i] + u[i+1]) / dx2
    end
    du[{N+1}] = epsilon * (2u[{N}] - 2u[{N+1}]) / dx2
end
...
uStorage = np.zeros((N + 1, timesteps + 1, TOTAL_SAMPLES))
...
# consecutive differences: u(t+1) - u(t), shape (65, 500, 2000)
y = uStorage[:, 1:, :] - uStorage[:, :-1, :]

# first KM moment (drift estimate), shape (65, 500)
xdt = (1 / dt) * np.mean(y, axis=2)

# second KM moment (diffusion squared estimate), shape (65, 500)
...
print("uStorage shape:", uStorage.shape)  # (65, 501, 2000)
print("xdt shape:", xdt.shape)  # (65, 500)
print("xdiff shape:", xdiff.shape)  # (65, 500)
