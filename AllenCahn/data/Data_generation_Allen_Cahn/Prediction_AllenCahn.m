...
save('../Allen_cahn_prediction.mat', 'sol_true', 'sol_pred')

%%
figure();
subplot(4,1,1); imagesc(mean(sol_true, 3))
subplot(4,1,2); imagesc(mean(sol_pred, 3))
subplot(4,1,3); imagesc(abs(mean(sol_true, 3) - mean(sol_pred, 3))) % fixed bug
subplot(4,1,4); imagesc(std(sol_pred, 0, 3)) % fixed MATLAB std syntax